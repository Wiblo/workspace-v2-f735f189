// Adapted from react-grab (MIT) for phase-3 drag multi-select.
import {
...
const filterElementsInDrag = (
  dragRect: DragRect,
  isValidSelectableElement: (element: Element) => boolean,
  ...
  for (const candidateElement of candidates) {
    ...
    if (!isValidSelectableElement(candidateElement)) continue
...
export const getElementsInDrag = (
  ...
  const elements = filterElementsInDrag(dragRect, isValidSelectableElement, strict)
